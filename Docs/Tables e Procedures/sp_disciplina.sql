...
/****** Object:  StoredProcedure [dbo].[sp_disciplina]    Script Date: 29/09/2025 01:57:37 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_disciplina](@op CHAR(1), @codigoDisc INT, @codigoCurso INT, @nome VARCHAR(70),
							@inicio TIME, @horasSemanais INT, @conteudos VARCHAR(MAX), 
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					(@codigoDisc, @codigoCurso, @nome, @inicio, @horasSemanais, @conteudos)
				...
				BEGIN
					...
					BEGIN
						UPDATE disciplina
						SET codigoCurso = @codigoCurso, nome = @nome, inicio = @inicio, 
							horasSemanais = @horasSemanais, conteudos = @conteudos
