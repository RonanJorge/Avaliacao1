...
/****** Object:  StoredProcedure [dbo].[sp_aluno]    Script Date: 29/09/2025 01:56:22 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_aluno](@op CHAR(1), @cpf BIGINT, @nome VARCHAR(100), @nomeSocial VARCHAR(100),
						   ...
						   @anoIngresso INT, @semestreIngresso INT, @codigocurso INT, @anoLimite INT, @semestreLimite INT,
						   @ra VARCHAR(9),
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					@emailCorporativo, @conclusaoEM, @anoIngresso, @semestreIngresso, @codigoCurso,
				...
				BEGIN
					...
					BEGIN
						...
						SET nome = @nome, nomeSocial = @nomeSocial, nascimento = @nascimento, 
							...
							anoIngresso = @anoIngresso, semestreIngresso = @semestreIngresso, codigoCurso = @codigocurso,
